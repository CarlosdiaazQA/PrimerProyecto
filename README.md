# PrimerProyecto

Este es mi primer proyecto utilizando Python, Git y GitHub.

## Sobre este proyecto

Este repositorio contiene mis primeros ejercicios realizados durante mi aprendizaje de programación en Python.

## Programas incluidos

- `hola.py` - Uso de variables, entrada de datos y condicionales.
- `contador.py` - Práctica con ciclos.
- `menu.py` - Menú interactivo con diferentes opciones.
- `notas.py` - Evaluación de notas utilizando condicionales y ciclos.

## Tecnologías utilizadas

- Python
- Git
- GitHub
- Visual Studio Code

## Objetivo

Desarrollar mis conocimientos de programación y control de versiones, avanzando progresivamente hacia proyectos relacionados con QA y automatización de pruebas.